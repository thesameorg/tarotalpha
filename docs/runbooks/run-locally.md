...
4. Открыть `http://localhost:5173/?asset=BTCUSDT`: график грузится живым запросом к бирже из браузера — сначала Bybit, потом Binance; если биржа недоступна из сети — страница скажет об этом, а не покажет пустой график.

Что ещё гоняют руками:

- `pnpm run test` — движок, биржевые адаптеры на моках и воркер внутри workerd.
- `pnpm run typecheck && pnpm run lint && pnpm run build` — типы, линтер и сборка; весь прогон CI на PR, с тестами, prettier и политикой документирования, — команда `/check` (`docs/adr/0004-checks-on-pr-only.md`).
