...
3. Телефон: тот же бот в мобильном клиенте. Жест вниз по графику при странице в верхнем положении сворачивает мини-апп — если мешает, `disableVerticalSwipes()` в `web/telegram.ts`.

## Без Telegram

`http://localhost:5173/#tgWebAppPlatform=web&tgWebAppVersion=9.0&tgWebAppThemeParams=%7B%22bg_color%22%3A%22%23161824%22%7D` — скрипт грузится, сайт считает себя мини-аппом: переключатели ушли из шапки, вызовы к клиенту уходят в пустоту; `bg_color` задаёт `colorScheme` (тёмный фон — тёмная), без него скрипт считает тему светлой. `&tgWebAppStartParam=<id>` в том же хэше проверяет дип-линк. `sessionStorage` помнит запуск до закрытия вкладки — обычный сайт снова в новой вкладке.
