...
import { READERS_EN } from "./readers-en";
...
export const en: Dictionary = {
  ...
  paid: {
    ...
    invite: (mana: number): string => `Invite an initiate · +${String(mana)} each`,
    welcomed: (mana: number): string => `You were brought in: +${String(mana)} mana`,
  },
  paywall: {
    ...
    payWith: "Pay from wallet",
    byHand: "Send it by hand",
    address: "Wallet",
    comment: "Comment",
    commentWarn: "Without the comment the payment cannot be found. Wallets call this field comment, memo or note.",
    openWallet: "Open wallet",
    waiting: "Waiting for the payment",
    credited: (mana: number): string => `In the purse: ${String(mana)}`,
    endless: "Your purse no longer runs out",
    failed: "That did not work. Try again",
    meditating: "The payment module is still meditating",
    titleShort: "Not enough mana",
    titleTopUp: "Buy mana",
    house: "house pick",
    endlessLot: "A purse that never runs out",
    paid: "Credited",
    lead: "Every day of a reading costs mana, and the further ahead, the more: the future is harder to see. Mana trickles back every hour and refills completely with a new day.",
    buy: "Buy",
    back: "Back to the reading",
  },
  fan: { hint: "Pull three cards", close: "close" },
  cloth: "Open a tarot reading — learn the fate of the candles",
  reader: {
    unscored: "Not scored yet",
    rated: (stars: number, of: number): string => `rated ${String(stars)} of ${String(of)}`,
    wins: (pct: number): string => `Closest of the five in ${String(pct)} % of readings`,
    others: "Everyone at the table",
    ...
    askShort: (cost: number): string => `Not enough mana: ${String(cost)} needed`,
  ...
  reading: {
    ...
    horizonEnd: (days: number): string =>
      `${String(days)} days is the whole horizon of a reading; the cards look no further`,
  ...
  mine: {
    button: "My readings",
    ripe: "ripe",
    days: (n: number): string => `days: ${String(n)}`,
    opinions: (n: number): string => `second opinions: ${String(n)}`,
  ...
  readerName: (id: ReaderId): string => READERS_EN[id].name,
  readerBlurb: (id: ReaderId): string => READERS_EN[id].blurb,
