...
export const ru = {
  ...
  manaPanel: {
    refill: "Восстанавливается по одной в час и целиком к местной полуночи.",
    full: "Бак полный",
    fullAt: (time: string): string => `Полный бак в ${time}`,
  },
  paid: {
    title: "Экстра-мана",
    order: "Тратится после обычной",
    what: "Покупается за настоящие деньги. Сама не восполняется и не сгорает: лежит, пока не потратите.",
    invite: (mana: number): string => `Позвать посвящённого · по +${String(mana)} обоим`,
    welcomed: (mana: number): string => `Вас привели: +${String(mana)} маны`,
  },
  paywall: {
    slow: "Оплачено. Мана вот-вот придёт",
    scan: "Наведите камеру кошелька",
    backToReading: "К раскладу",
    exact: "Сумма точно",
    wasTon: "бывший TON",
    pick: "Чем платите?",
    payWith: "Заплатить кошельком",
    byHand: "Перевести вручную",
    address: "Кошелёк",
    comment: "Комментарий",
    commentWarn: "Без комментария платёж не найдётся. В кошельках это поле зовут comment, memo или note.",
    openWallet: "Открыть кошелёк",
    waiting: "Ждём платёж",
    credited: (mana: number): string => `В кошельке: ${String(mana)}`,
    endless: "Кошелёк больше не кончается",
    failed: "Не вышло. Попробуйте ещё раз",
    meditating: "Платёжный модуль ещё медитирует",
    titleShort: "Маны не хватает",
    titleTopUp: "Купить ману",
    house: "ставка дома",
    endlessLot: "Кошелёк, который не кончается",
    paid: "Зачислено",
    lead: "Каждый день расклада стоит маны, и чем он дальше, тем дороже: будущее видно хуже. Мана понемногу возвращается каждый час, а с новым днём — целиком.",
    buy: "Купить",
  ...
  prophecy: {
    legend: "Пустые бирюзовые и розовые свечи — прогноз карт, плотные зелёные и красные — рынок",
