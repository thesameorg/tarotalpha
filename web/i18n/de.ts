...
export const de: Dictionary = {
  ...
  paid: {
    ...
    invite: (mana: number): string => `Eingeweihten einladen · +${String(mana)} für beide`,
    welcomed: (mana: number): string => `Du wurdest gebracht: +${String(mana)} Mana`,
  },
  paywall: {
    ...
    payWith: "Mit Wallet zahlen",
    byHand: "Von Hand überweisen",
    address: "Wallet",
    comment: "Kommentar",
    commentWarn:
      "Ohne Kommentar lässt sich die Zahlung nicht finden. Wallets nennen dieses Feld comment, memo oder note.",
    openWallet: "Wallet öffnen",
    waiting: "Warten auf die Zahlung",
    credited: (mana: number): string => `Im Beutel: ${String(mana)}`,
    endless: "Dein Beutel wird nicht mehr leer",
    failed: "Hat nicht geklappt. Bitte noch einmal",
    meditating: "Das Zahlungsmodul meditiert noch",
    titleShort: "Mana reicht nicht",
    titleTopUp: "Mana kaufen",
    house: "Wahl des Hauses",
    endlessLot: "Ein Beutel, der nie leer wird",
    paid: "Gutgeschrieben",
  ...
  reader: {
    ...
    readBy: (name: string): string => `Den Tag lesen wie ${name}`,
  },
  reading: {
  ...
  mine: {
    ...
    ripe: "reif",
    days: (n: number): string => `Tage: ${String(n)}`,
    opinions: (n: number): string => `zweite Meinungen: ${String(n)}`,
  ...
  summaryBy: (name: string): string => `Der Tag in einer Zeile · ${name}.`,
  how: "Methode",
  contact: "Kontakt",
