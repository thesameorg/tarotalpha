...
export const pt: Dictionary = {
  ...
  paid: {
    ...
    invite: (mana: number): string => `Convidar um iniciado · +${String(mana)} para ambos`,
    welcomed: (mana: number): string => `Você foi trazido: +${String(mana)} de mana`,
  },
  paywall: {
    ...
    payWith: "Pagar pela carteira",
    byHand: "Enviar à mão",
    address: "Carteira",
    comment: "Comentário",
    commentWarn:
      "Sem o comentário o pagamento não é encontrado. As carteiras chamam este campo de comment, memo ou note.",
    openWallet: "Abrir carteira",
    waiting: "À espera do pagamento",
    credited: (mana: number): string => `Na bolsa: ${String(mana)}`,
    endless: "Sua bolsa não acaba mais",
    failed: "Não resultou. Tente outra vez",
    meditating: "O módulo de pagamento ainda medita",
    titleShort: "Mana insuficiente",
    titleTopUp: "Comprar mana",
    house: "escolha da casa",
    endlessLot: "Uma bolsa que nunca acaba",
    paid: "Creditado",
  ...
  cloth: "Abra uma leitura de tarô e descubra o destino dos candles",
  reader: {
    ...
    others: "Todos à mesa",
    current: "Lê as cartas para você",
    locked: (name: string): string => `${name} conduz esta leitura até o último dia`,
    ...
    askShort: (cost: number): string => `Mana insuficiente: faltam ${String(cost)}`,
    ...
    readBy: (name: string): string => `Ler o dia como ${name}`,
  },
  reading: {
    ...
    horizonEnd: (days: number): string =>
      `${String(days)} dias são todo o horizonte de uma leitura; as cartas não olham além`,
  ...
  scroll: {
    ...
    foretell: (id: string, asset: string): string =>
      `Certifica-se que a leitura ${id} promete ao mercado de ${asset} o que está desenhado abaixo`,
    stage: { forecast: "previsão", interim: "intermediário", final: "resultado" },
    drawn: (stage: string, at: string): string => `${stage} · folha composta em ${at}`,
  ...
  summaryBy: (name: string): string => `O dia em uma linha · ${name}.`,
  how: "Método",
  contact: "Contato",
